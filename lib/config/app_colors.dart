part of 'helper.dart';

abstract final class AppColor {
  static const Color primary = Color(0xffEA8913);
  static const Color primaryEC9529 = Color(0xffEC9529);
  static const Color whiteFFFFFF = Color(0xffFFFFFF);
  static const Color pinkFF2147 = Color(0xffFF2147);
  static const Color blue046EC4 = Color(0xff046EC4);
  static const Color colorF3F3F3 = Color(0xffF3F3F3);
  static const Color colorAEAEAE = Color(0xffAEAEAE);
  static const Color color0B0B0B = Color(0xff0B0B0B);
  static const Color color21203F = Color(0xff21203F);
  static const Color colorFEF6EC = Color(0xffFEF6EC);
  static const Color colorF6F6F6 = Color(0xffF6F6F6);
  static const Color color212121 = Color(0xff212121);
  static const Color color121212 = Color(0xff121212);
  static const Color colorFAFAFA = Color(0xffFAFAFA);
  static const Color color1E2430 = Color(0xff1E2430);
  static const Color color8E8E8E = Color(0xff8E8E8E);
  static const Color colorF8F8F8 = Color(0xffF8F8F8);
  static const Color colorBBBBBB = Color(0xffBBBBBB);
  static const Color colorB7B7B7 = Color(0xffB7B7B7);
  static const Color color5A5A5A = Color(0xff5A5A5A);
  static const Color color7A7A7A = Color(0xff7A7A7A);
  static const Color colorBCBDBD = Color(0xFFbcbdbd);
  static const Color black000000 = Color(0xff000000);
  static const Color blackF7F8FB = Color(0xffF7F8FB);
  static const Color transparent = Colors.transparent;
  static const Color black4A4A4A = Color(0xff4A4A4A);
  static const Color black929292 = Color(0xff929292);
  static const Color black232323 = Color(0xff232323);
  static const Color grey040F2529 = Color(0xff040F25);
  static const Color blue303744 = Color(0xff303744);


  static const Color orangeFFF9F5 = Color(0xffFFF9F5);
  static const Color secondry = Color(0xffFFECD6);
  static const Color orangeFff9f0 = Color(0xfffff9f0);
  static const Color orangeF285000F = Color(0xfffff8f0);
  static const Color orangeD7d3d0 = Color(0xffd7d3d0);
  static const Color orange878787 = Color(0xff878787);
  static const Color orangeFFECD6 = Color(0xffFFECD6);
  static const Color greyAAA6B9 = Color(0xffAAA6B9);
  static const Color grey999999 = Color(0xff999999);
  static const Color orangeEA8913 = Color(0xffEA8913);
  static const Color black160C08 = Color(0xff160C08);
  static const Color black111111 = Color(0xff111111);
  static const Color greyEDEDED = Color(0xffEDEDED);
  static const Color greyDBDBDB = Color(0xffDBDBDB);
  static const Color greyD9D9D9 = Color(0xffD9D9D9);

  static const Color grey646464 = Color(0xff646464);
  static const Color grey99ABC6 = Color(0xff99ABC6);
  static const Color redF75454 = Color(0xffF75454);
  static const Color greyEAEAEA = Color(0xffEAEAEA);
  static const Color green = Color(0xff34C759);
  static const Color blackF1F1F1 = Color(0xffF1F1F1);
  static const Color color7D8B98 = Color(0xff7D8B98);
  static const Color colorDDDDDD = Color(0xffDDDDDD);
  static const Color color465573 = Color(0xff465573);
}
